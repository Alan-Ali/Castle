...
$answer = $_GET['answer'];
$answer = str_replace('"', "\042", $answer);
$answer = str_replace("'", "''", $answer);

    $DetailLen = strlen($answer);
    ...
    $content = substr($answer, 0, $DetailDev);
    $content1 = substr($answer, $DetailDev, $DetailDev);
    $content2 = substr($answer, $DetailDev * 2, $DetailDev);
    $content3 = substr($answer, $DetailDev * 3, ($DetailDev + 1));
...
$sql = "INSERT INTO POST(Post_Id, User_Id, Post_Parent_Id, Post_Level, Post_Root, Post_Content,Post_Date, Post_Content1, Post_Content2, Post_Content3)
                VALUES('$fullpostid', '$userid', '$parentid' , 1, '$parentid', '$content', '$time', '$content1','$content2','$content3')"; // for the first level paarentid and root will be same
